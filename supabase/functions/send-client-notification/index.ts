...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { client_id, is_reminder = false } = await req.json();
    ...
    if (postsErr) {
      ...
      return json({ error: "Failed to query posts" }, 500);
    }

    if (!approvalPosts || approvalPosts.length === 0) {
      return json({ error: "No posts awaiting client approval" }, 404);
    }
    ...
    if (recipients.length === 0) {
      return json({ error: "No client email on file" }, 404);
    }
