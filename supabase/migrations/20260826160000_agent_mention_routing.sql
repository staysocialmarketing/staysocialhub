-- Extend comment notification trigger to route agent mentions to Telegram.
-- When a comment mentions an ss_agent user, calls agent-mention-notify
-- edge function for each tagged agent so NanoClaw can wake the right container.
...
BEGIN
  ...
  -- Clients only on public comments
  ...
  -- Mention notifications for non-agent mentions not already notified
  IF array_length(v_mention_ids, 1) IS NOT NULL THEN
    ...
    WHERE mid != NEW.user_id
      ...
      -- Skip agent accounts — they get Telegram notifications instead
      AND NOT EXISTS (
        SELECT 1 FROM public.user_roles ur
         WHERE ur.user_id = mid AND ur.role = 'ss_agent'
      )
      AND NOT EXISTS (
        SELECT 1 FROM public.notification_preferences np
  ...
  -- ── Agent mention routing via Telegram ────────────────────────────
  v_service_key := COALESCE(
    current_setting('supabase.service_role_key', true),
    current_setting('app.settings.service_role_key', true)
  );

  IF v_service_key IS NOT NULL AND array_length(v_mention_ids, 1) IS NOT NULL THEN
    FOR v_agent IN
      SELECT u.id, u.name
        FROM public.users u
        JOIN public.user_roles ur ON ur.user_id = u.id
       WHERE ur.role = 'ss_agent'
         AND u.id = ANY(v_mention_ids)
         AND u.id != NEW.user_id
    LOOP
      BEGIN
        PERFORM net.http_post(
          url := 'https://ktyjtbivycjkklkrcudb.supabase.co/functions/v1/agent-mention-notify',
      ...
      EXCEPTION WHEN others THEN
        RAISE WARNING 'agent-mention-notify call failed for %: %', v_agent.name, SQLERRM;
      END;
    END LOOP;
  END IF;
