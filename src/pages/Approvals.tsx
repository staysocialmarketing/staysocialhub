...
import {
  MessageSquare, Calendar, Image as ImageIcon, Hash, Clock, CheckCircle,
} from "lucide-react";
    ...
    <div className="space-y-1.5">
      <Tooltip>
        ...
        <TooltipContent side="right" className="max-w-[250px]">
          <p className="text-xs line-clamp-3">{post.caption ? post.caption.substring(0, 120) + (post.caption.length > 120 ? "…" : "") : "No caption"}</p>
  ...
  const { data: posts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .in("status_column", ["internal_review", "client_approval", "scheduled", "published"])
...
function ClientApprovals() {
  ...
  const { data: posts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .in("status_column", ["client_approval", "scheduled", "published"])
        .order("created_at", { ascending: false });
      if (error) throw error;
      return data || [];
    },
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Content for Approval with action buttons */}
      <section>
        <h3 className="text-lg font-semibold text-foreground mb-3 flex items-center gap-2">
