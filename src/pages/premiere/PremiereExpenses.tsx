...
import {
  ...
  TrendingUp, TrendingDown, DollarSign, Receipt, Building2, Loader2,
} from "lucide-react";
    ...
    >
      ...
      {uploading ? (
        <div className="flex flex-col items-center gap-2">
          <Loader2 className="h-8 w-8 text-[#C5A258] animate-spin" />
          <p className="text-sm text-muted-foreground">Parsing and uploading...</p>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-2">
          <Upload className="h-8 w-8 text-muted-foreground/50" />
          <p className="text-sm font-medium text-foreground">Drop QuickBooks GL export here</p>
          <p className="text-xs text-muted-foreground">.xlsx or .csv · Click to browse</p>
        </div>
      )}
    </div>
  );
...
export default function PremiereExpenses() {
  ...
  const { data: expenses = [], isLoading: expensesLoading } = useQuery({
    queryKey: ["premiere-expenses", selectedUploadId],
    queryFn: async () => {
      let q = supabase.from("premiere_expenses").select("*").order("transaction_date", { ascending: false });
      if (selectedUploadId !== "all") q = q.eq("upload_id", selectedUploadId);
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (result.expenses.length === 0) {
        toast.error("No valid expense rows found in this file. Check the format.");
        return;
      }
      ...
      // Batch insert expenses (200 at a time)
      const expenseBatches: ParsedExpense[][] = [];
      for (let i = 0; i < result.expenses.length; i += 200) {
        expenseBatches.push(result.expenses.slice(i, i + 200));
      }
      ...
      toast.success(`Uploaded ${result.expenses.length} transactions from ${file.name}`);
  ...
  // Monthly trend data
  const monthlyData = filtered.reduce<Record<string, number>>((acc, e) => {
    if (e.amount <= 0) return acc;
    const month = e.transaction_date.slice(0, 7); // YYYY-MM
  ...
  const categoryTotals = filtered.reduce<Record<string, number>>((acc, e) => {
    if (e.amount > 0) acc[e.category] = (acc[e.category] || 0) + e.amount;
    return acc;
  }, {});
    ...
    <div className="flex flex-col gap-6 p-6 max-w-screen-xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Upload QuickBooks GL exports to track advertising & digital spend</p>
        </div>
        {hasData && (
          <Button
            variant="outline"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="flex flex-col gap-2">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider px-0.5">Upload History</p>
          {uploadsLoading ? (
            <div className="flex items-center justify-center h-20 text-muted-foreground">
              <Loader2 className="h-4 w-4 animate-spin" />
            </div>
          ) : uploads.length === 0 ? (
            <p className="text-sm text-muted-foreground p-3">No uploads yet</p>
          ) : (
            <div className="flex flex-col gap-1.5 max-h-40 overflow-y-auto">
              {uploads.map(u => (
                <div
                  key={u.id}
                  onClick={() => setSelectedUploadId(u.id === selectedUploadId ? "all" : u.id)}
                  className={`
                    flex items-center gap-2 p-2.5 rounded-lg border cursor-pointer transition-all text-xs
                    ${selectedUploadId === u.id
                      ? "border-[#C5A258]/60 bg-[#C5A258]/5 text-foreground"
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] opacity-60">{u.row_count} rows · {u.date_range_start && fmtDate(u.date_range_start)}</p>
                  </div>
                  <button
                    onClick={(e) => { e.stopPropagation(); deleteMutation.mutate(u.id); }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          <Card className="lg:col-span-3 border-border/60">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-foreground">Monthly Spend</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="lg:col-span-2 border-border/60">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-foreground">Vendor Breakdown</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
        ...
        <div className="flex flex-col sm:flex-row gap-2 flex-wrap">
          ...
          {(search || dateFrom || dateTo || selectedCategory !== "all") && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => { setSearch(""); setDateFrom(""); setDateTo(""); setSelectedCategory("all"); }}
                        ...
                        <td className="px-4 py-3">
                          <Badge
                            ...
                            style={{
                              ...
                              border: `1px solid ${CATEGORY_COLORS[e.category] || "#94a3b8"}30`,
                            }}
                          >
                            {e.category}
                          </Badge>
                        </td>
                        <td className={`px-4 py-3 font-semibold tabular-nums whitespace-nowrap ${e.amount < 0 ? "text-green-500" : "text-foreground"}`}>
        ...
        !uploadsLoading && (
          <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Drop a QuickBooks GL detail report above (.xlsx or .csv)</p>
            </div>
          </div>
        )
      )}
